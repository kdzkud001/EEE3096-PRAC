...
uint32_t val =0;//Initialise adc value to 0
...
int main(void)
{
  ...
  while (1)
  {
	...
	//Converting adc_val into a string called str
	char str[10];
...
void writeLCD(char *char_in){
    delay(3000);
	lcd_command(CLEAR);
	// Write the value obtained from the ADC
...
uint32_t pollADC(void){
  // TODO: Complete function body to get ADC val
	...
	//Obtain the most recent value from the ADC
	val = HAL_ADC_GetValue(&hadc);
	//Stop the ADC
	HAL_ADC_Stop(&hadc);
	//Return the most recent value from the ADC
	return val;
}
